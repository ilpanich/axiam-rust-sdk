...
mod tests {
    ...
    //
    // `tests/fixtures/v2_reference_vectors.json` is GENERATED by the AXIAM
    // server sign path (`axiam-amqp::messages`) and vendored into this repo
    // verbatim. It is the ground truth every SDK's HMAC implementation must
    // reproduce exactly (CONTRACT.md §8 "Canonical reference vectors"). It
    // carries, per message type, the exact canonical signed JSON bytes, the
    // resulting hex HMAC, and the HKDF-derived per-tenant subkey used to
    // compute it. Re-vendor it from the server repo whenever §8 changes.
    const REFERENCE_VECTORS_JSON: &str =
        include_str!("../../tests/fixtures/v2_reference_vectors.json");
