...
let verifier = JwksVerifier::new(http.clone(), &base_url)?
    .expect_tenant_id(tenant_uuid)
    .with_revocation_feed(RevocationFeed::new(http, &base_url)?);
```

**It is not a control, and every property follows from that.** It is off unless you
attach one. It is never consulted over the network on the request path — `verify` reads
a cached set and, at most, starts a refresh whose result the *next* caller sees. And it
**never fails closed**: an unreachable feed, a non-`200`, a body that does not parse, or
an `alg` this build does not know all behave exactly as no feed at all — not as an empty
list, which would assert that nothing has been revoked and is a guard silently honouring
no revocations while appearing to honour them. Every §10.1 rule runs first and still
...
(`RevocationFeed::with_poll_interval`); the cached set is bounded, and a document larger
than the bound is treated as unusable rather than truncated — a truncated set is a guard
...
let client = AxiamClient::builder()
    ...
    .org_slug("globex")
    .build()?;
client.login("root@example.com", &password).await?;
```
...
workspace *before* the tenant's OPAQUE mode is read, so the `404` that means "OPAQUE is
not offered here" never arrives and this crate has no fallback to take. Sign-in then fails
even against a tenant with OPAQUE disabled, reported as `invalid credentials`, which sends
a user off to reset a password that works.

`build()` refuses a blank `tenant_slug` or `org_slug` so that never reaches the wire —
`""` is exactly as much of a tenant as none at all. It is checked at `build()` rather than
in `tenant_slug()`, which returns `Self` and has nowhere to put an error.
...
A TLS listener decides whether to ask for a client certificate during the handshake, before it
has seen a byte of HTTP, so "request one on `/oauth2/token` but not on `/oauth2/authorize`" is
not something a single listener can do. A deployment that wants both runs two — and
`mtls_endpoint_aliases` in the discovery document is how the second one is named.

Once the client above is built with `with_client_cert`, every request it makes presents that
certificate, so the §12 helpers **prefer the alias** over the top-level entry of the same name
wherever the document publishes one:

| Operation | Endpoint aliased |
|---|---|
| `oidc_exchange`, `oidc_refresh`, `login_client_credentials`, `device_poll`, `token_exchange`, `uma_exchange_ticket` | `token_endpoint` |
...
- **An absent `mtls_endpoint_aliases` is not an error.** It means "this deployment terminates
  mutual TLS on the issuer's own host", so the conventional endpoints keep being used. A
  deployment running `client_auth = optional` on one listener serves both populations there and
  correctly publishes nothing. The same holds one level in: every field of
  `MtlsEndpointAliases` is `Option`, and an endpoint the object does not name falls back rather
...
- **No alias is ever synthesised.** Only the six endpoints RFC 8705 §5 lists can be aliased —
  ...
  front-channel and the third is public key material; sending a browser to an mTLS host raises a
  native certificate-chooser dialog most users cannot answer.
- **`issuer` does not move.** It is an identifier, not an endpoint. §12.4 rule 3 still requires a
  token's `iss` to equal the document's `issuer` by exact string comparison, including for a
  token minted at an alias endpoint — the expected issuer is never derived from the host that was
  called.
...
`mtls_endpoint_aliases` is read only by a client that actually presents a
certificate, and an alias that cannot carry one fails the call rather than
quietly reverting to the top-level endpoint. Falling back looks like the safe
answer and is the dangerous one: the caller asked to authenticate with a
...
- **A scheme weaker than the endpoint it replaces.** An alias substitutes for
  exactly one top-level endpoint, so that is what it is compared against. An
  `http` alias for an `https` `token_endpoint` is a downgrade; an `http` alias
  for an `http` endpoint is a development deployment, not a misconfiguration.

A client built **without** `with_client_cert` never reads the member at all —
not even to validate it — so a deployment whose aliases are malformed cannot
break the clients that never use them.

An alias's query component is preserved rather than appended to: the
`tenant_id` the caller authenticated against displaces the published one, and
every other parameter survives exactly as the server wrote it.
